...
def CesarCifr(mensaje,n): #Funcion Cesar , recibe el mendasaje a cifrar y en n , este es la clave de cesar 
    cifrado =""
    for l in mensaje: 
            if l in abc:
                i = abc.find(l)
...
def vigenereDescif(mensaje,clave):
    cifrado = ""
    for i in range(0,len(mensaje)):
        des = get_desp(clave[i % len(clave)]) 
        cifrado+=(CesarDes(mensaje[i],des)) 
    return cifrado
